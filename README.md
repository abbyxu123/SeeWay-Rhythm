...
SeeWay Rhythm 当前处于设计与规则整理阶段。第一版面向单用户、本地优先的个人研究场景：设备常显当前两小时时段的重点提示，并允许用户通过按键和语音提出具体问题。项目不以随机文案代替排盘；每个领域 Agent 可以参与解释、权衡和结论判断，但不能修改盘面事实，所有结论都必须来自可复现、可追溯的计算与规则证据。
...
SeeWay-Rhythm/
├── apps/
│   ├── device-firmware/     # ESP32-S3 固件
│   └── local-console/       # 本地配置、核验和问事界面
├── packages/
│   ├── calendar-core/       # 历法、节气、干支和时区
│   ├── qimen-core/          # 奇门排盘与判断
│   ├── ziwei-core/          # 紫微背景
│   ├── rule-engine/         # 规则匹配与冲突处理
│   └── contracts/           # 设备通信和结果结构
├── firmware-assets/         # 字体、图标和单色屏资源
├── tests/                   # 规则样例与边界测试
└── docs/plans/              # 产品和实施设计
```
...
项目尚未进入功能实现。当前工作重点是把资料转化为明确、可测试、带来源的规则规格，然后再搭建计算核心和设备固件。

详细设计见 [产品与系统设计](docs/plans/2026-08-20-seeway-rhythm-design.md)和[多智能体推演平台设计](docs/plans/2026-08-21-multi-agent-platform-design.md)。
