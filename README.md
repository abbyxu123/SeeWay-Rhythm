...
SeeWay Rhythm 当前已经完成多 Agent 可信底座，正在进入时间口径与计算规则整理阶段。第一版面向单用户、本地优先的个人研究场景：设备常显当前两小时时段的重点提示，并允许用户通过按键和语音提出具体问题。项目不以随机文案代替排盘；每个领域 Agent 可以参与解释、权衡和结论判断，但不能修改盘面事实，所有结论都必须来自可复现、可追溯的计算与规则证据。
...
后续将在同一工作区增加 `time-core`、`qimen-core`、设备固件和本地控制台。
...
可信推演中台已经可以校验请求、路由一个主 Agent、建议但不擅自启用辅助 Agent、执行最小权限授权、保留独立报告与冲突，并控制结果是否持久化。所有领域计算器当前仍标记为 `unverified`；实际执行会明确返回 `unsupported`，不会用通用文案冒充术数结论。

下一里程碑是锁定时间与奇门流派口径，建立人工核对的固定金样，再实现 `time-core` 与 `qimen-core`。只有计算器、规则证据和边界测试全部通过，对应 Agent 才能改为 `available`。
